...
En desarrollo, ``spacy.load("es_core_news_sm")`` funciona porque el modelo es
un paquete Python instalado. Pero en el ejecutable congelado, spaCy no lo
resuelve como paquete: los datos quedan en ``sys._MEIPASS/es_core_news_sm/
es_core_news_sm-<ver>``. Este loader prueba ambos caminos.
"""
...
_MODELOS = ("es_core_news_sm", "es_core_news_md", "es_core_news_lg")
_cache = {}
...
def _rutas_candidatas(nombre: str):
    ...
    for base in bases:
        ...
        if subdirs:
            yield subdirs[-1]
        # o, en algunos empaquetados, directamente en la carpeta
...
def cargar_modelo_es():
    ...
    # 1) intento normal (paquete instalado, modo desarrollo)
